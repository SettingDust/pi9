...
1. Call `subagent` with `request: { action: "spawn", spawns: [...] }`, using one `spawns` entry per requested child and the `recursive-test` agent.
...
4. Join every run you started in one `subagent` call with `request: { action: "join", subagentIds: [...] }` before answering.
