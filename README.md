...
| Pane-owned execution | Runs child generations in mux panes through `packages/subagent/src/pane-execution.ts`, with Herdr placement, steer/cancel controls, retained child session files, activity sidecars, automatic terminal-pane closure, and read-only historical pane reopen. | Upstream executes through generic Generation runtime boundaries without pane ownership. |
