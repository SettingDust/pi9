...
| Pane-owned execution, Herdr placement, automatic terminal-pane closure, read-only historical pane reopen, launcher failure sidecars, and atomic launcher prompts | Not present upstream; upstream executes through generation runtime boundaries. | Migrated to the Generation adapter. Keep pane-specific logic in `pane-execution.ts` / `pane-child.ts` and keep upstream core changes limited to generic controls, activity observation, retained session files, and disposal hooks. |
